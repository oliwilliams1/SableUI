...
void SableUI::TextFieldComponent::Init(State<InputFieldData>& data, const ElementInfo& p_info, bool multiline)
{
	info = p_info;
	externalState = &data;
		...
		if (el)
		{
			...
			if (clickedInside)
			{
				if (!dataCopy.isFocused)
				{
					...
					cursorVisible.set(true);
					m_cursorBlinkInterval.Start(500);
					change = true;
				}
			}
			else
